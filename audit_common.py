...
def safe_lifetime(dgms) -> float:
    """Maximum finite H1 lifetime from a ripser diagram list, safely returning 0."""
...
def symmetrized_knn_graph(X: np.ndarray, k: int):
    ...
    Critical reviewer-facing convention:
...
def geodesic_distance_matrix(
    ...
    *,
    ...
    Default reviewer-safe behavior is ``sentinel_fill=False``: a disconnected
...
def h1_lifetime_from_distance_matrix(
    ...
    *,
    ...
    """Compute max finite H1 lifetime from a distance matrix with percentile threshold."""
    if ripser is None:
    ...
    positive = D_work[(D_work > eps) & np.isfinite(D_work)]
    thresh = float(np.percentile(positive, pct)) if positive.size else 1.0
...
def geodesic_h1_lifetime(
    ...
    *,
    ...
    L = h1_lifetime_from_distance_matrix(D, pct=pct, eps=eps, rescale=False)
