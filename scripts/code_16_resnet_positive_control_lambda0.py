...
log_step_invariant(cos_t, sin_t)   # Runtime invariant check.
...
for amp_frac in amp_fracs:
    ...
    for b in block_sizes:
        # Determine null count based on block size and targeted mode.
        if TARGETED_MODE:
        ...
        for i in range(n_nulls):
            ...
            del D_null
            gc.collect()
        # compute_stats returns formal_trigger (pperm < alpha) separately
        # from nominal_trigger. Use formal_trigger for inference.
...
print("FINAL RESULTS")
print("=" * 90)
# Primary summary uses formal_trigger, not nominal_trigger.
# The fallback column is included as a diagnostic audit field.
print(df[[
    "drift_scale",
    "amp_frac",
    "block_size",
...
# Explicit check: flag any row where fallback fired without formal_trigger.
