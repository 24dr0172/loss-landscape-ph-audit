...
def max_h1_geodesic_from_euclidean_D(
    ...
    """Complete-filtration graph-geodesic maximum H1 lifetime.

    The final release policy is fail-loud: disconnected kNN-geodesic
    graphs raise RuntimeError instead of using sentinel-filled distances. The
    complete filtration matches the block-diagnostic statistic in code_10.
    ``geodesic_pct`` is retained only for backward-compatible CLI/CSV schemas.
    """
    if D_full.shape[0] < 4:
        return 0.0
...
def run_calibration(config: Config) -> None:
    ...
    print(
        f"Geodesic           : k={config.geodesic_k}, "
        f"max_k={config.geodesic_max_k}, complete filtration "
        f"(legacy pct metadata={config.geodesic_pct})"
    )
