...
try:
    ...
    _dev = None
    raise RuntimeError("PyTorch/torchvision is required for the optimization stress-test audit.") from exc
# =============================================================================
# CONFIG
# =============================================================================
EPOCHS = 200
...
print("=" * 70)
print("Optimization stress-test neural full-space audit")
print("=" * 70)
...
# DISTANCE + PH
# =============================================================================
def step_gram(X: np.ndarray) -> np.ndarray:
    """Float64 Gram matrix of trajectory increments.

    Observed and permuted-null checkpoint distances are both derived from this
    one matrix. This avoids reconstructing null trajectories through float32
    cumulative sums and exactly matches the main neural-audit distance policy.
    """
...
def distance_from_step_gram(
    G: np.ndarray,
    perm: np.ndarray | None = None,
) -> np.ndarray:
    """Pairwise checkpoint distances for an optional increment ordering."""
    Gp = np.asarray(G, dtype=np.float64)
    if perm is not None:
        p = np.asarray(perm, dtype=int)
        Gp = Gp[np.ix_(p, p)]
    prefix = np.pad(
        np.cumsum(np.cumsum(Gp, axis=0), axis=1),
        ((1, 0), (1, 0)),
    )
    diag = np.diag(prefix)
...
def max_H1_euclidean_from_dist(D: np.ndarray) -> float:
    """Complete-filtration Euclidean VR maximum H1 lifetime."""
...
def _one_null_stats_from_gram(
    G: np.ndarray,
    permutation_seed: int,
) -> tuple[float, float]:
    """One matched-step null statistic pair from the shared increment Gram."""
    rng = np.random.default_rng(permutation_seed)
    perm = rng.permutation(G.shape[0])
    D_null = distance_from_step_gram(G, perm)
    return max_H1_euclidean_from_dist(D_null), _geodesic_from_dist(D_null)
...
def ambient_tests(
    ...
    """Observed and matched-step-null PH from one float64 increment Gram."""
    if len(X) > SUBSAMPLE_PH:
        raise ValueError(
            "This stress audit expects at most SUBSAMPLE_PH checkpoints; "
            "subsampling before increment permutation would change the null."
        )
    _, d = X.shape
    ns = seed + 100
    G = step_gram(X)
    D_obs = distance_from_step_gram(G)
    Lo_euc = max_H1_euclidean_from_dist(D_obs)
    Lo_geo = _geodesic_from_dist(D_obs)
    del D_obs
    ...
    if d > D_THRESH or n_jobs == 1:
        ...
        for i in range(n_nulls):
            H_euc[i], H_geo[i] = _one_null_stats_from_gram(G, ns + i)
            if i % 10 == 0:
                gc.collect()
    else:
        pairs = Parallel(n_jobs=n_jobs)(
            delayed(_one_null_stats_from_gram)(G, ns + i)
            for i in range(n_nulls)
        )
        H_euc = np.array(
            [p[0] for p in pairs],
...
class SmallCNN(nn.Module):
    """
    SmallCNN architecture used for the neural trajectory audits.
    """
    def __init__(self):
        super().__init__()
...
def get_loader(
    ...
    """
    CIFAR-10 loader.
    Training transform used for the neural trajectory audits:
...
def run_single(
    ...
    # -----------------------------------------------------------------------
    # Full-space geodesic audit
    # -----------------------------------------------------------------------
    print(f"  Full-space geodesic ({N_NULLS_FULL} nulls, sequential)")
    t0 = time.time()
    ...
    print(f"  PCA stability ({n_pca_nulls} nulls, {N_JOBS_PCA} jobs)")
...
def plot_phase_map(df: pd.DataFrame) -> None:
    ...
    ax.set_title("Stress-test phase map, epoch 200")
