...
print(f"Drift scale  : {drift_scale}  ← REAL RESNET DRIFT + LOOP")
...
print(f"  lambda=0 script used fracs: [1e-5, 3e-5, 1e-4, 3e-4, 1e-3, 3e-3, 1e-2]")
print(f"  lambda=1 script uses fracs: {[f'{x:.0e}' for x in amp_fracs]}")
print(f"  Extra large fracs (3e-2, 1e-1) test whether even high-amplitude loops")
print(f"  collapse under real ResNet drift.  Small fracs (1e-5, 3e-5) omitted")
print(f"  because drift >> loop at those scales makes injection meaningless.")
# ====================== HELPERS ======================
def mad(a):
    med = np.median(a)
...
for amp_frac in amp_fracs:
    ...
    for b in block_sizes:
        # [M2]  Null count depends on block size and targeted mode.
        if TARGETED_MODE:
        ...
        for i in range(n_nulls):
            ...
            # The loop is added to Yc BEFORE computing steps, so
            # null steps are loop-inclusive increments of the combined
            # (drift + loop) trajectory.  This correctly implements the
            # matched-step null from Section 4.3: each null is a random walk
            # built from the same increments as the observed trajectory, with
            # temporal order destroyed.  Both observed and null contain the
            # loop contribution in their step multisets.
            def null_iter():
                for start in range(0, d, chunk_cols):
                    end      = min(start + chunk_cols, d)
                    Xraw     = np.array(X[:, start:end], dtype=np.float32, copy=True)
                    ...
                    steps = Yc[1:] - Yc[:-1]   # loop-inclusive steps [Finding 4]
                    steps = steps[perm]
            ...
            del D_null
            gc.collect()
        # [M1]  Use formal_trigger as the primary decision variable.
...
print("FINAL RESULTS")
print("=" * 90)
# [M1]  Primary summary uses formal_trigger; fallback included for audit trail.
print(df[[
    "drift_scale",
    "amp_frac",
    "block_size",
    ...
    "geo_nominal_trigger",  # [M1] diagnostic
    "geo_zrob",
    "geo_zero_frac",
    "sentinel_nulls",       # [Finding 5]
]].to_string(index=False))
# [M1]  Explicit check: any row where fallback drives without formal_trigger?
