...
Decision convention
-------------------
Formal triggers are defined by pperm < ALPHA for this static-cloud null. The
...
print("Formal decision: pperm < ALPHA; separation_flag is diagnostic only")
...
def _build_geodesic(
    ...
    Graph construction failures are hard failures in the release version.
    We do not convert disconnected-geodesic failures into Lmax=0 rows.
    """
    if len(X) < 4:
        return None, k0
...
def _ph_from_dist(D: np.ndarray) -> tuple[float, int]:
    """
    Run Vietoris-Rips PH on a geodesic distance matrix.

    Returns:
        max_H1_lifetime, n_significant_bars

    Significant bar:
        lifetime > BAR_THRESH * max_H1_lifetime
    """
    if D is None:
        return 0.0, 0
    ...
    offdiag_positive = D[(D > EPS) & np.isfinite(D)]
...
def run_benchmark(
    ...
    return dict(
        ...
        geo_formal_trigger=stats["formal_trigger"],
        geo_nominal_trigger=stats["nominal_trigger"],
        N_SAMPLES=N_SAMPLES,
        SUBSAMPLE=SUBSAMPLE,
        N_NULLS=N_NULLS,
...
def main() -> pd.DataFrame:
    ...
    if not all_pass:
        ...
        raise RuntimeError(
            "Manifold benchmark validation failed.\n"
