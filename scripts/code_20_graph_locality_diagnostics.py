...
_REPO_ROOT = _RepoPath("/home/shivam/tda_final_execution").resolve()
...
def pairwise_distance_chunked(path: Path, chunk_cols: int) -> np.ndarray:
    """Pairwise Euclidean distances for an n x d trajectory using column chunks.

    This is observed-trajectory only. For n=200, the returned matrix is tiny;
    chunking keeps the ResNet-scale d dimension manageable.
    """
