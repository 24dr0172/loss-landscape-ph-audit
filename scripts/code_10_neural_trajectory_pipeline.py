...
def _kinematic_ratio_chunked(tp: Path) -> dict:
    ...
    return dict(path_length=path_len, net_displacement=net_disp,
                kinematic_R=path_len / max(net_disp, 1e-8))
def _gram_chunked(X_mmap, start_row: int = 0, n_rows: int = None) -> np.ndarray:
    n_total, d = X_mmap.shape
    if n_rows is None:
        n_rows = n_total - start_row
    sq   = np.zeros(n_rows, dtype=np.float64)
    gram = np.zeros((n_rows, n_rows), dtype=np.float64)
    buf  = np.empty((n_rows, CHUNK_COLS), dtype=np.float32)
    for start in range(0, d, CHUNK_COLS):
        end  = min(start + CHUNK_COLS, d)
        cols = end - start
        Xc   = buf[:, :cols]
        Xc[:] = X_mmap[start_row:start_row + n_rows, start:end]
        if _CUDA_OK:
            Xc_t   = torch.from_numpy(Xc).to(_dev)
            sq    += (Xc_t * Xc_t).sum(dim=1).cpu().numpy().astype(np.float64)
            gram  += torch.mm(Xc_t, Xc_t.t()).cpu().numpy().astype(np.float64)
            del Xc_t
        else:
    ...
    return np.sqrt(np.maximum(sq[:, None] + sq[None, :] - 2.0 * gram, 0.0))
def _null_gram_chunked(X_mmap, idx: int, master: int,
                       block_size: int = 1) -> np.ndarray:
    """
    Now accepts block_size (default 1 = matched-step).
    Block-permuted-step null distance matrix via column chunks.
    RAM budget per chunk (reusing pre-allocated buffers):
        steps_buf  : CHUNK_COLS × (n-1) × float32  = 80 MB
        null_buf   : CHUNK_COLS × n     × float32  = 80 MB
        GPU tensor : same                           = 80 MB  (freed each iter)
        gram       : n × n × float64               = negligible (200×200)
    """
    n, d  = X_mmap.shape
    ...
    for start in range(0, d, CHUNK_COLS):
        ...
        cols = end - start
        Xc = X_mmap[:, start:end].astype(np.float32)
        sc = steps_buf[:, :cols]
        nc = null_buf[:,  :cols]
        ...
        del Xc
        if _CUDA_OK:
            nc_t   = torch.from_numpy(nc).to(_dev)
            sq    += (nc_t * nc_t).sum(dim=1).cpu().numpy().astype(np.float64)
            gram  += torch.mm(nc_t, nc_t.t()).cpu().numpy().astype(np.float64)
            del nc_t
        else:
            nc64  = nc.astype(np.float64)
...
def _ph_from_dist(D: np.ndarray,
                  ...
                  geodesic_pct: float | None = None) -> tuple[float, float]:
    """Return (max_H1_euc, max_H1_geo) from a precomputed distance matrix.
    Optional geodesic_k / geodesic_pct override globals for Stage 5."""
    pct    = GEODESIC_PCT if geodesic_pct is None else float(geodesic_pct)
    iu     = np.triu_indices_from(D, k=1)
    thresh = float(np.percentile(D[iu], pct)) if len(D[iu]) else 1.0
    dgms   = ripser(D, maxdim=1, distance_matrix=True, thresh=thresh)["dgms"]
    if len(dgms) >= 2 and len(dgms[1]):
        H1  = dgms[1]; fin = H1[np.isfinite(H1[:, 1])]
        euc = float(np.max(fin[:, 1] - fin[:, 0])) if len(fin) else 0.0
    else:
        euc = 0.0
    geo = _geodesic_from_dist(D.copy(),
...
def ambient_tests_chunked(tp: Path, seed: int, n_nulls: int,
    ...
    """
    Accepts block_size (default 1 = matched-step).
    Accepts geodesic_k / geodesic_pct for Stage 5 sensitivity sweep.
    """
    X_mmap = np.load(tp, mmap_mode='r')
    ns     = seed + 100
    D_obs           = _gram_chunked(X_mmap)
    Lo_euc, Lo_geo  = _ph_from_dist(D_obs,
                                     geodesic_k=geodesic_k,
                                     geodesic_pct=geodesic_pct)
    del D_obs; gc.collect()
    H_euc = np.zeros(n_nulls)
    H_geo = np.zeros(n_nulls)
    for i in range(n_nulls):
        D_null = _null_gram_chunked(X_mmap, i, ns, block_size=block_size)
        H_euc[i], H_geo[i] = _ph_from_dist(D_null,
                                             geodesic_k=geodesic_k,
                                             geodesic_pct=geodesic_pct)
        del D_null
        gc.collect()
...
def _pca_gram_chunked(tp: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    for start in range(0, d, CHUNK_COLS):
        ...
        mean[start:end] = X_mmap[:, start:end].mean(axis=0)
    mean = mean.astype(np.float32)
    gram = np.zeros((n, n), dtype=np.float64)
    for start in range(0, d, CHUNK_COLS):
        end = min(start + CHUNK_COLS, d)
        Xc  = X_mmap[:, start:end].astype(np.float32) - mean[start:end]
        if _CUDA_OK:
            Xc_t   = torch.from_numpy(Xc).to(_dev)
            gram  += torch.mm(Xc_t, Xc_t.t()).cpu().numpy().astype(np.float64)
            del Xc_t
...
def _gram_dist(X: np.ndarray) -> np.ndarray:
    ...
    return np.sqrt(np.maximum(sq + sq.T - 2.0 * (X64 @ X64.T), 0.0))
# =============================================================================
# GPU ACCELERATION
# =============================================================================
...
def _gram_dist_gpu(X: np.ndarray) -> np.ndarray:
    if not _CUDA_OK or X.shape[1] <= GPU_D_THRESH:
        return _gram_dist(X)
    X_t  = torch.from_numpy(X.astype(np.float32)).to(_dev)
    sq   = (X_t * X_t).sum(dim=1, keepdim=True)
    gram = torch.mm(X_t, X_t.t())
    D    = torch.sqrt(torch.clamp(sq + sq.t() - 2.0 * gram, min=0.0))
...
def _gpu_null_dist_loop(X: np.ndarray, n_nulls: int,
    ...
    if not _CUDA_OK:
        return None
    X_t     = torch.from_numpy(np.ascontiguousarray(X, dtype=np.float32)).to(_dev)
    steps_t = X_t[1:] - X_t[:-1]
    start_t = X_t[:1]
    ns      = seed + 100
    n_steps = len(steps_t)
    ...
    for i in range(n_nulls):
        ...
        perm_np = _block_permutation_indices(n_steps, block_size, rng)
        perm = torch.from_numpy(perm_np.astype(np.int64)).to(_dev)
        null_t = torch.vstack(
            [start_t, start_t + torch.cumsum(steps_t[perm], dim=0)])
        sq   = (null_t * null_t).sum(dim=1, keepdim=True)
        gram = torch.mm(null_t, null_t.t())
        D    = torch.sqrt(torch.clamp(sq + sq.t() - 2.0 * gram, min=0.0))
        dists.append(D.cpu().numpy().astype(np.float64))
    return dists
...
def max_H1_euclidean(X: np.ndarray, cap: int, rng: np.random.Generator,
                     geodesic_pct: float | None = None) -> float:
    """Optional geodesic_pct overrides the filtration threshold."""
...
def ambient_tests(X: np.ndarray, seed: int,
    ...
    ns   = seed + 100
    if d > D_THRESH:
        gpu_dists = _gpu_null_dist_loop(X, n_nulls, seed, block_size=block_size)
        if gpu_dists is not None:
            H_geo = np.array([
                _geodesic_from_dist(gpu_dists[i],
                                     geodesic_k=geodesic_k,
                                     geodesic_pct=geodesic_pct)
                for i in range(n_nulls)])
            H_euc = np.array([
                max_H1_euclidean(
                    _make_null(X, i, ns, block_size=block_size), SUBSAMPLE_PH,
            ...
            H_geo = np.array([
                max_H1_geodesic(
                    ...
                    geodesic_k=geodesic_k, geodesic_pct=geodesic_pct)
                for i in range(n_nulls)])
    else:
        nulls = [
            _make_null(X, kk, ns, block_size=block_size)
            for kk in range(n_nulls)
        ]
        H_euc = np.array(Parallel(n_jobs=n_jobs)(
            delayed(max_H1_euclidean)(
                nulls[i], SUBSAMPLE_PH, np.random.default_rng(seed + 200 + i),
                geodesic_pct=geodesic_pct)
            for i in range(n_nulls)))
        H_geo = np.array(Parallel(n_jobs=n_jobs)(
            delayed(max_H1_geodesic)(
                nulls[i], SUBSAMPLE_PH, np.random.default_rng(seed + 400 + i),
                geodesic_k=geodesic_k, geodesic_pct=geodesic_pct)
            for i in range(n_nulls)))
    Lo_euc = max_H1_euclidean(X, SUBSAMPLE_PH, np.random.default_rng(seed + 10),
                               geodesic_pct=geodesic_pct)
    Lo_geo = max_H1_geodesic(X, SUBSAMPLE_PH, np.random.default_rng(seed + 300),
                              geodesic_k=geodesic_k, geodesic_pct=geodesic_pct)
    return compute_stats(Lo_euc, H_euc), compute_stats(Lo_geo, H_geo)
# =============================================================================
# PROJECTION TEST
...
def classify_geodesic(s_geo: dict, context: str = "matched_step") -> str:
    ...
    ``robust_recurrence``. A seed-level or diagnostic-grid p-value is not a
    final recurrence claim until the manuscript table builder applies seed
...
def run_neural_sensitivity_single(arch: str, opt: str,
    ...
    FIX (vs document): X_full is loaded ONCE per seed (outside the
    k/pct loop) to avoid 12 redundant disk reads per seed.
    FIX: uses N_JOBS_BLOCK (=4) for normal-d path, matching Stage 4.
    FIX: uses N_NULLS_SENS_CHUNKED (=50) for ResNet-18 to limit runtime.
    """
    rows: list[dict] = []
    tp = TRAJ_DIR / f"{arch}__{opt}__seed{seed}.npy"
    if not tp.exists():
...
def _save_outputs(df: pd.DataFrame) -> None:
    ...
    main_cols = [
        "arch", "optimizer", "seed", "rep", "space", "pca_d",
        "geo_Lobs", "geo_zrob", "geo_pperm", "geo_formal_trigger",
        "geo_zero_frac", "geo_null_collapsed", "decision",
...
def _plot_phase_map(df: pd.DataFrame) -> None:
    ...
    colours = {
        "robust_recurrence": "#2ca02c",
        "no_recurrence":     "#1f77b4",
    }
    markers = {
        "mlp2_mnist":       "o",
        "smallcnn_cifar10": "s",
...
# =============================================================================
# EXPERIMENT 0c — SYNTHETIC LOOP SANITY CHECK
# =============================================================================
...
def run_sanity_circle(d_embed: int = 2048, n_cycles: int = CIRCLE_N_CYCLES,
    ...
    decision = "robust_recurrence" if s_geo["formal_trigger"] else "FAILED_SANITY"
...
def run_sanity_negcontrol() -> dict:
    ...
    _, s_geo = ambient_tests(X, seed, n_nulls=N_NULLS_FULL, n_jobs=N_JOBS_FULL)
    dec_time = time.time() - t0
    decision = "no_recurrence" if not s_geo["formal_trigger"] else "FALSE_POSITIVE"
    print(f"    zrob={s_geo['zrob']:+.2f}  p={s_geo['pperm']:.4f}  "
...
def run_experiment_0c() -> pd.DataFrame:
    ...
    print()
    print("=" * 70)
    print("  EXPERIMENT 0c — Synthetic Loop Sanity Check")
    ...
    print("\n  Test D: negative control — random walk (expect no_recurrence)")
    rows.append(run_sanity_negcontrol())
    df = pd.DataFrame(rows)
    print("\n  Sanity check summary:")
    for _, row in df.iterrows():
        status = "pass" if row["decision"] in (
            "robust_recurrence", "no_recurrence") else "FAIL"
        print(f"    [{status}]  {row['label']:30s}  => {row['decision']}")
    all_pass = all(
        row["decision"] in ("robust_recurrence", "no_recurrence")
        for _, row in df.iterrows())
    print()
    if all_pass:
        print("  All sanity checks passed — Experiment 4 results are interpretable.")
    else:
        print("  SANITY FAILURE — investigate before trusting Experiment 4.")
...
if __name__ == "__main__":
    if not SKIP_MAIN:
        ...
        if not sanity_ok:
            raise RuntimeError(
                "Experiment 4 sanity check failed. Refusing to run neural trajectory audit."
            )
        else:
            df = run_experiment()
        ...
        print("\n[SKIP_MAIN=1] Skipping Experiments 0c and 4.")
    if RUN_BLOCK_ROBUSTNESS:
        df_block = run_block_robustness()
    else:
        print("\n[RUN_BLOCK_ROBUSTNESS=0] Skipping Stage 4 (block-permutation "
              "robustness). Set RUN_BLOCK_ROBUSTNESS=1 to enable.")
    if RUN_NEURAL_SENSITIVITY:
        df_sens = run_neural_sensitivity()
    else:
        print("\n[RUN_NEURAL_SENSITIVITY=0] Skipping Stage 5 "
              "(neural k / threshold sensitivity). "
              "Set RUN_NEURAL_SENSITIVITY=1 to enable.")
