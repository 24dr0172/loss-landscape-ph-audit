...
def top_finite_h1_bar(dgms) -> np.ndarray | None:
    ...
    if len(finite) == 0:
        return None
    return finite[np.argmax(finite[:, 1] - finite[:, 0])]
...
def euclidean_ph(X: np.ndarray):
    """
    Ambient Euclidean PH on the given point cloud.
    Uses a 95th-percentile filtration threshold, matching the audit style.
    """
    thresh = adaptive_euclidean_thresh(X, EUCLIDEAN_PCT)
    kwargs = {"maxdim": 1}
    if thresh is not None:
        kwargs["thresh"] = thresh
    return ripser(X, **kwargs)["dgms"]


def max_h1_euclidean(X: np.ndarray) -> float:
    return safe_lifetime(euclidean_ph(X))
...
def geodesic_ph(X: np.ndarray):
    ...
    return dgms, D_scaled, final_k, sentinel_used


def max_h1_geodesic(X: np.ndarray) -> float:
    dgms, _, _, _ = geodesic_ph(X)
    return safe_lifetime(dgms)
...
def main() -> None:
    ...
    # Observed PH
    # -------------------------------------------------------------------------
    dgms_euc = euclidean_ph(helix3)
    dgm0_euc = dgms_euc[0]
    dgm1_euc = dgms_euc[1] if len(dgms_euc) > 1 else np.empty((0, 2))

    dgms_geo, _, final_k, sentinel_used = geodesic_ph(helix3)
    dgm0_geo = dgms_geo[0]
    dgm1_geo = dgms_geo[1] if len(dgms_geo) > 1 else np.empty((0, 2))

    Lobs_euc = safe_lifetime(dgms_euc)
    Lobs_geo = safe_lifetime(dgms_geo)

    top_h1_euc = top_finite_h1_bar(dgms_euc)
    ...
    if len(dgm1_geo) > 0 and stats_geo["Lobs"] > 0:
        # Show up to three longest finite H1 bars, scaled into the visible range.
        finite_geo_h1 = dgm1_geo[np.isfinite(dgm1_geo[:, 1])]
        if len(finite_geo_h1) > 0:
            lives = finite_geo_h1[:, 1] - finite_geo_h1[:, 0]
            show_h1 = finite_geo_h1[np.argsort(lives)[-3:]]
            for b, d in show_h1:
                ax_C.barh(
                    h1_c,
